...
function createREADME(dirname, name){
  let curDir = fs.readdirSync(resolve(rootDir, name, dirname))
  ...
  // 写入文件
  fs.writeFileSync(resolve(rootDir, name, dirname, 'index.md'), str)
}


export default function (name) {
  // 获取文件夹并创建README.md文件
  const webArr = fs.readdirSync(resolve(rootDir, name))
